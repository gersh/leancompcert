...
def coq_check_text(
    ...
    reg_defs = ", ".join(f"Cert._v_{i}" for i in range(1, reg_count + 1))
    scratch_regs = ", ".join(
        ["Cert._v_0"] + [f"Cert._v_{i}" for i in range(1, reg_count + 1)])
    counter_regs = ", ".join(
        [f"Cert._v_{i}" for i in range(1, counter + 1)])
    base_defs = ", ".join(
        [f"Cert._v_{base}", "Cert._v_0", f"Cert._v_{counter}"]
    ...
    if syntax_only:
        return "\n".join(lines)
    lines += [
        "Lemma imported_scalar_layout_wf :",
        "  emitter_layout_wf imported_scalar_layout",
        ...
        f"    unfold {scratch_regs}.",
        "    simpl; intuition congruence.",
        f"  - change (~ In Cert._v_{counter} [{regs}]).",
        f"    unfold {counter_regs}.",
        "    simpl; intuition congruence.",
        f"  - change (Cert._v_0 <> Cert._v_{counter}).",
        f"    unfold Cert._v_0, Cert._v_{counter}; congruence.",
        "Qed.",
        "",
        ...
        f"    (aes_reg_count Source.{emitter}).",
        "Proof.",
        "  unfold array_emitter_layout_wf.",
        "  split; [exact imported_scalar_layout_wf|].",
        "  repeat split.",
        f"  - change (Cert._v_{base} <> Cert._v_0).",
        f"    unfold {base_defs}; congruence.",
        f"  - change (Cert._v_{base} <> Cert._v_{counter}).",
        f"    unfold {base_defs}; congruence.",
        f"  - change (~ In Cert._v_{base} [{regs}]).",
        f"    unfold {base_defs}; simpl; intuition congruence.",
        "Qed.",
        "",
