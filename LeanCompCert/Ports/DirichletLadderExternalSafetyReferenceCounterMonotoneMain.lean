...
/-! Acceptance-directed bounds through historical flags 9--17. -/
...
def historicalMainFlags (c : Cfg) : List AInstr :=
...
theorem historicalMainFlags_machine_bounds
    (c : Cfg) (idx n : Nat) (s : AState)
    (hv : s.regs rViol = n) (hgate : s.regs 48 ≤ 1)
    (hNoWrap : n + 8 < M) :
    n ≤ (arun idx s (historicalMainFlags c)).regs rViol ∧
      (arun idx s (historicalMainFlags c)).regs rViol ≤ n + 8 := by
  let s1 := arun idx s (historicalFlag9 c)
  let s2 := arun idx s1 (historicalFlag10 c)
  ...
  have b1 := historicalFlag9_machine_bounds c idx n s hv (by omega)
  ...
  have g1 : s1.regs 48 ≤ 1 := by
    ...
    rw [LeanCompCert.Verified.ArrayRegFrame.arun_frame idx 48
      (historicalFlag9 c) (by rfl) s]
    exact hgate
  have b2 := historicalFlag10_machine_bounds c idx (s1.regs rViol) s1 rfl g1
    (by omega)
  ...
  have hout : arun idx s (historicalMainFlags c) = s8 := by
    simp only [historicalMainFlags, arun_append, s1, s2, s3, s4, s5, s6, s7,
      s8]
  rw [hout]
  constructor <;> omega
