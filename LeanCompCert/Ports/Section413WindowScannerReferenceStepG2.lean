import LeanCompCert.Ports.Section413WindowBodyReferenceSoundG2
import LeanCompCert.Ports.Section413WindowScannerFoldSound

/-!
# One compiled G2 scanner step refines one paper slot
...
namespace LeanCompCert.Ports.Section413WindowScannerReferenceStepG2
...
open LeanCompCert.Ports.Section413WindowBodyReferenceSoundG2
open LeanCompCert.Ports.Section413WindowSchedule

set_option maxRecDepth 100000 in
theorem g2_scanner_step_core (k : Nat) (s p e t : AState)
  ...
  have hev := g2_body_slot k p hk hword harray hsched harr hflag hadd hr
  rw [← he] at hev
  have rowFrame (r : Nat)
      (hw : LeanCompCert.Verified.ArrayRegFrame.writes r
        (LeanCompCert.Ports.Section413WindowRowCheck.body g2Cfg.checkLo
          g2Cfg.offset) = false) : t.regs r = e.regs r := by
    rw [ht]
...
set_option maxRecDepth 100000 in
theorem g2_scanner_step (k : Nat) (s : AState)
    (hk : k < productionRows * slots)
    ...
    (htadd : (checkedState k s g2Cfg).regs
      LeanCompCert.Ports.Section413SignedAdd.rViol = 0)
    (hr : EventBodyReceipts k (scheduledState k s) g2Cfg) :
    Accumulates s (checkedState k s g2Cfg)
      (LeanCompCert.Ports.Section413WindowPairingBridge.k1SlotDelta
        g2G 2 (slotAt k).n (k % slots)).lo
      (LeanCompCert.Ports.Section413WindowPairingBridge.k1SlotDelta
        g2G 2 (slotAt k).n (k % slots)).hi
  ...
  let e := eventedState k s g2Cfg
  let t := checkedState k s g2Cfg
  ...
  have hparr : p.arr =
      (LeanCompCert.Ports.Section413G2TableProgram.rawFinal g2TableCfg).arr := by
  ...
  exact g2_scanner_step_core k s p e t hk hpword hparray hsched hparr hflag
    hpadd hr rfl rfl
...
#print axioms g2_scanner_step_core
#print axioms g2_scanner_step

end LeanCompCert.Ports.Section413WindowScannerReferenceStepG2
