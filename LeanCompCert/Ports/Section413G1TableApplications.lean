...
`Section413G1TableSound` into the head-mass and main-term model Boolean.  It
...
namespace LeanCompCert.Ports.Section413G1TableApplications

open LeanCompCert.Ports.Section413Sweep
open LeanCompCert.Ports.Section413G1Sound (g1Prefix)
...
theorem hmPrefix_succ (R N k : Nat) :
    hmPrefix R N (k + 1) =
      hmStep R (weightV1 R (k + 1)) (hmPrefix R N k) (k + 1) := by
  unfold hmPrefix
  rw [List.range_succ, List.foldl_append]
  rfl

theorem hmPrefix_full (R N : Nat) : hmPrefix R N N = hmRun 1 R N := by
...
theorem g1Prefix_hmPrefix_core (R checkLo N k : Nat) :
    (g1Prefix R checkLo N k).acc = (hmPrefix R N k).acc ∧
      (g1Prefix R checkLo N k).g = (hmPrefix R N k).g := by
  induction k with
  | zero => exact ⟨rfl, rfl⟩
  | succ k ih =>
      rw [LeanCompCert.Ports.Section413G1Sound.g1Prefix_succ,
        hmPrefix_succ]
      simp only [g1Step, hmStep]
      rw [ih.1, ih.2]
      exact ⟨rfl, rfl⟩
...
def hmV1TableOK (T : Array Cell) (N : Nat) : Bool :=
  let s := hmTablePrefix T N
  decide (s.1 ≤ 37224400 * (SCALE : Int)) &&
    decide (10000000 * s.2 ≤ 2 * 3624829 * (SCALE : Int))

theorem hmV1OK_of_table (R checkLo N : Nat) (T : Array Cell)
    (htable : ∀ X, 1 ≤ X → X ≤ N →
      T[X]! = (g1Prefix R checkLo N X).g)
    (hok : hmV1TableOK T N = true) : hmV1OK R N = true := by
  have hagg := hmPrefix_aggregates_eq_table R checkLo N T htable N (Nat.le_refl _)
  rw [hmV1OK, ← hmPrefix_full]
  unfold hmV1TableOK at hok
  rw [hagg.1, hagg.2]
  exact hok

end LeanCompCert.Ports.Section413G1TableApplications
