...
`Ports/Section413G1Program.lean` states two open obligations.  This file
attacks the second,

```text
(2)  c.tFlag = 0 → g1SweepOK c.rounds c.checkLo c.cap = true
...
4. **Phase 1, phase 2, the run.**  `sweep_prefix` relates every candidate
   boundary to `g1Prefix`; `tFlag_zero_sound` reads the final Boolean.
...
namespace LeanCompCert.Ports.Section413G1Sound
...
theorem Admissible.toDenote {c : Cfg} (h : Admissible c) :
    LeanCompCert.Ports.Section413G1Denote.Admissible c where
  capPos := h.cap_ge_two
...
scale unit. -/
theorem weightV1_natAbs_le_scale (R X : Nat) (hX : 1 ≤ X) :
    (LeanCompCert.Ports.Section413Sweep.weightV1 R X).lo.natAbs ≤ SCALE ∧
      (LeanCompCert.Ports.Section413Sweep.weightV1 R X).hi.natAbs ≤ SCALE := by
...
theorem tweight_live_weight (R X sig mc : Nat) (z : TSel) (t : TState)
    ...
    (⟨decodeZ (tweight z t).wLo, decodeZ (tweight z t).wHi⟩ :
      LeanCompCert.Ports.Section413Sweep.Cell) =
      LeanCompCert.Ports.Section413Sweep.weightV1 R X := by
  have hmc := muCode_cases X R
  rw [← hcode] at hmc
  ...
  rw [LeanCompCert.Ports.Section413Sweep.weightV1, ← hsigRef, hrat]
  rcases hmc with rfl | rfl | rfl
  · simp [tweight, hW, hX, hXmod, harr, hsigma, hsigNe,
      Nat.mod_eq_of_lt hsigM, bnat, SCALE_eq_sweep,
...
theorem tstep_phase2_mu_frame (c : Cfg) (hc : Admissible c) (idx : Nat)
  ...
  have htrial : (ttrial z t).arr n = t.arr n := by
    unfold ttrial
    simp [hDL, Nat.ne_of_gt hn]
  have hp := LeanCompCert.Ports.Section413G1Denote.Admissible.passBProducts
...
theorem tstep_weight_spec (c : Cfg) (hc : Admissible c) (X : Nat)
    ...
    (⟨decodeZ (c.tstep idx t).wLo, decodeZ (c.tstep idx t).wHi⟩ :
      LeanCompCert.Ports.Section413Sweep.Cell) =
      LeanCompCert.Ports.Section413Sweep.weightV1 c.rounds X := by
  let x := X - 1
...
/-- The scheduled weight round in the transparent run computes `weightV1`.
...
theorem weight_square_capped_spec (c : Cfg) (hc : Admissible c) (X : Nat)
  ...
  let W := LeanCompCert.Ports.Section413Sweep.weightV1 c.rounds X
  have hw :
  ...
  have hWcap := weightV1_natAbs_le_scale c.rounds X hX
...
def g1Prefix (R checkLo N k : Nat) :
    LeanCompCert.Ports.Section413Sweep.G1State :=
  (List.range k).foldl
    (fun s i => LeanCompCert.Ports.Section413Sweep.g1Step R checkLo s (i + 1))
    ⟨Array.replicate (N + 1) LeanCompCert.Ports.Section413Sweep.czero,
      LeanCompCert.Ports.Section413Sweep.czero, true⟩

theorem g1Prefix_zero (R checkLo N : Nat) :
    g1Prefix R checkLo N 0 =
      ⟨Array.replicate (N + 1) LeanCompCert.Ports.Section413Sweep.czero,
        LeanCompCert.Ports.Section413Sweep.czero, true⟩ := rfl

theorem g1Prefix_succ (R checkLo N k : Nat) :
    g1Prefix R checkLo N (k + 1) =
      LeanCompCert.Ports.Section413Sweep.g1Step R checkLo
        (g1Prefix R checkLo N k) (k + 1) := by
  unfold g1Prefix
  rw [List.range_succ, List.foldl_append]
  rfl

theorem g1Prefix_full (R checkLo N : Nat) :
    g1Prefix R checkLo N N =
      LeanCompCert.Ports.Section413Sweep.g1Run R checkLo N := rfl
...
structure SweepRel (c : Cfg) (t : TState)
    (s : LeanCompCert.Ports.Section413Sweep.G1State) : Prop where
...
theorem phase1_sweepRel (c : Cfg) (hc : Admissible c) :
    SweepRel c (tRunUpto c c.phase1) (g1Prefix c.rounds c.checkLo c.cap 0) := by
  ...
  · refine ⟨by simp [g1Prefix], ?_⟩
    ...
    rw [hp.arr_above _ h1, hp.arr_above _ h2,
      hzeroDecode]
    simp only [g1Prefix, List.range_zero, List.foldl_nil]
    rw [getElem!_pos
  ...
  · unfold Capped
    ...
    simp
  · simp only [g1Prefix, List.range_zero, List.foldl_nil, hp.gLo, hp.gHi,
...
theorem SweepRel.step (c : Cfg) (hc : Admissible c) (hflag : c.tFlag = 0)
    ...
    (hrel : SweepRel c (tRunUpto c (c.phase1 + k * c.p))
      (g1Prefix c.rounds c.checkLo c.cap k)) :
    SweepRel c (tRunUpto c (c.phase1 + (k + 1) * c.p))
      (g1Prefix c.rounds c.checkLo c.cap (k + 1)) := by
  let X := k + 1
  let s := g1Prefix c.rounds c.checkLo c.cap k
  let W := LeanCompCert.Ports.Section413Sweep.weightV1 c.rounds X
  ...
  change SweepRel c (tRunUpto c (c.phase1 + X * c.p))
    (LeanCompCert.Ports.Section413Sweep.g1Step c.rounds c.checkLo s X)
  have hok :
      (LeanCompCert.Ports.Section413Sweep.g1Step c.rounds c.checkLo s X).ok =
        true := by
    unfold LeanCompCert.Ports.Section413Sweep.g1Step
    dsimp only [W, ad]
    rw [hrel.ok_true]
  ...
  · simpa only [LeanCompCert.Ports.Section413Sweep.g1Step, W, ad] using hfin.1
  · simpa only [LeanCompCert.Ports.Section413Sweep.g1Step, W, ad] using
      hfin.2.2.2.2.2.2.2.1

theorem sweep_prefix (c : Cfg) (hc : Admissible c) (hflag : c.tFlag = 0) :
    ∀ k, k ≤ c.cap →
      SweepRel c (tRunUpto c (c.phase1 + k * c.p))
        (g1Prefix c.rounds c.checkLo c.cap k) := by
  intro k
  induction k with
  | zero =>
      intro _
...
module `Section413G1Verified` joins it to `g1Program_denote` and the verified
compiler theorem without introducing an import cycle.
-/

end LeanCompCert.Ports.Section413G1Sound
