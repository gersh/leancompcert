import LeanCompCert.Ports.Section413G1Program
...
/-- `Section413G1Program.guardBody`, as a scalar block with its cap
abstracted. -/
...
/-- `Section413G1Program.smDecomp`, as a scalar block with its two bounds
abstracted. -/
...
/-- `Section413G1Program.cmpLtBody`, as a scalar block.  No literal in it is
...
/-- `Section413G1Program.selTriple`, as a scalar block. -/
...
theorem g1Program_denote (c : Cfg) (hc : Admissible c) :
    (g1Program c).denote = some (c.tFlag)
```
