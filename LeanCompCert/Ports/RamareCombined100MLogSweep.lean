...
/-- Raw modular form of the complete log candidate.  All non-cumulative
increment arithmetic is still proved exactly; only the two carried endpoint
additions remain reduced modulo `M`. -/
theorem candidateBody_run_mod (k : Nat) (s : AState)
  ...
  have huLogU : u.regs rLogU = s.regs rLogU := by
    rw [scalarBody_frame k rLogU upperScalarBody (by decide) l,
      scalarBody_frame k rLogU lowerScalarBody (by decide) s]
  have hc := commitScalarBody_run_mod k u
  change out.regs rLogL =
      (u.regs rLogL + u.regs 11 * u.regs rIL) % M ∧
