...
theorem classAdvance_base
    ...
    (hbase : s.regs rBase = 1) :
    (arun k s c.markAdvanceBody).regs rBase = 1 := by
  have ha := c.markAdvanceBody_run k s hpre.values hpre.pow_ne_zero
    hpre.seg_sentinel hpre.mark_steps hpre.viol hpre.vmark
  dsimp only at ha
  rw [ha.2.2.1, h10, h25, hbase]
  simp [Cfg.stepPrime, Cfg.bumpPower, advanceActive, nextBaseValue]
