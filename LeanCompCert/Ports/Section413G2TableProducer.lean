...
This is the parity-gated companion of `Section413G1TableProducer`.  It runs
the verified arithmetic and table-store body without comparing its output to
a production-sized literal array.  A compiled scanner consumes the resulting
memory directly.
-/

namespace LeanCompCert.Ports.Section413G2TableProducer
...
def producerProgram (c : Cfg) : AProgram :=
  { regCount := LeanCompCert.Ports.Section413G2TableProgram.regCount
...
theorem producerProgram_wf (c : Cfg) : (producerProgram c).WF := by
  refine ⟨show rViol < LeanCompCert.Ports.Section413G2TableProgram.regCount
...
/-- Symbolic execution from the zero array.  The production fold remains
opaque; the proof is induction through the generic array-loop theorem. -/
...
theorem producer_output_eq_tFlag (c : Cfg) (hc : TableAdmissible c) :
  ...
  (LeanCompCert.Ports.Section413G2TableSound.rawFinal_core_tRun c hc).1

/-- Pointwise source meaning of the compiled producer's table, conditional
only on its ordinary clean-output result. -/
...
theorem producer_compiled_zero_sound (c : Cfg) (hc : TableAdmissible c)
  ...
  have hrViol : rViol < (producerProgram c).regCount := by
    simp [producerProgram, rViol,
      LeanCompCert.Ports.Section413G2TableProgram.regCount]
