...
/-! ## Generic padded-final scalar schedules -/

/-- Concrete non-list side conditions for the ordinary padded-final route.
The final historical window still occupies a full machine segment, but only
`historicalFinalValid row` candidates belong to the root table. -/
def historicalPaddedScheduleScalar (row : Row) : Prop :=
  let c := rowCfg row
  let b := historicalBootFuel row
  let l := historicalLaterFuel row
  let valid := historicalFinalValid row
  (row.rootCount ≠ 1 ∧ row.bootBound < row.rootCap ∧
    c.firstPrime ≤ c.segLen ∧
    c.rootCap + 1 < (row.bootBound + 1) * (row.bootBound + 1) ∧
    row.rootCount = b + 2 + l ∧ 0 < valid ∧ valid < c.segLen) →
  b * c.period ≤ c.rootSpan - 1 ∧ 1 + b * c.segLen < M ∧
  2 ≤ 1 + c.segLen - 1 ∧
  1 + b * c.segLen - 1 ≤ row.bootBound ∧
  b * c.period + c.period ≤ c.rootSpan - 1 ∧
  ...
  (b + 1) * c.period + l * c.period + c.period = c.rootSpan ∧
  laterBase c b + l * c.segLen + c.segLen < M ∧
  laterBase c b + l * c.segLen + valid - 1 = c.rootCap ∧
  laterBase c b + l * c.segLen + valid <
    (row.bootBound + 1) * (row.bootBound + 1) ∧
  mainBase c b l c.wDelta = c.lo

instance (row : Row) : Decidable (historicalPaddedScheduleScalar row) := by
  unfold historicalPaddedScheduleScalar
  infer_instance

def historicalPaddedSchedulesOK : Bool :=
  rows.all fun row => decide (historicalPaddedScheduleScalar row)

set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
theorem historicalPaddedSchedules_ok : historicalPaddedSchedulesOK = true := by
  decide

theorem row_historicalPaddedScheduleScalar (row : Row) (hrow : row ∈ rows)
    (hroot : row.rootCount ≠ 1) (hbootCap : row.bootBound < row.rootCap)
    (hpLen : (rowCfg row).firstPrime ≤ (rowCfg row).segLen)
    (hcover : (rowCfg row).rootCap + 1 <
      (row.bootBound + 1) * (row.bootBound + 1))
    (hcount : row.rootCount = historicalBootFuel row + 2 +
      historicalLaterFuel row)
    (hvalidPos : 0 < historicalFinalValid row)
    (hvalidLt : historicalFinalValid row < (rowCfg row).segLen) :
    let c := rowCfg row
    let b := historicalBootFuel row
    let l := historicalLaterFuel row
    let valid := historicalFinalValid row
    ...
    laterBase c b + l * c.segLen + valid <
      (row.bootBound + 1) * (row.bootBound + 1) ∧
    mainBase c b l c.wDelta = c.lo := by
  have hbool := (List.all_eq_true.mp historicalPaddedSchedules_ok) row hrow
  exact (of_decide_eq_true hbool)
    ⟨hroot, hbootCap, hpLen, hcover, hcount, hvalidPos, hvalidLt⟩
...
theorem row_historicalPaddedProductionSchedule (row : Row) (hrow : row ∈ rows)
    (hroot : row.rootCount ≠ 1) (hbootCap : row.bootBound < row.rootCap)
    (hpLen : (rowCfg row).firstPrime ≤ (rowCfg row).segLen)
    (hcover : (rowCfg row).rootCap + 1 <
      (row.bootBound + 1) * (row.bootBound + 1))
    ...
    (hvalidPos : 0 < historicalFinalValid row)
    (hvalidLt : historicalFinalValid row < (rowCfg row).segLen) :
    PaddedProductionCoreSchedule (rowCfg row) row.bootBound
      (historicalBootFuel row) (historicalLaterFuel row)
      (historicalFinalValid row) row.segCount (rowCfg row).wDelta := by
  let c := rowCfg row
  let bootFuel := historicalBootFuel row
  let laterFuel := historicalLaterFuel row
  have hcSeg : c.segLen = row.segLen := rfl
  ...
  have hn := row_historicalRegularFacts row hrow hroot hbootCap hpLen hcover
  let valid := historicalFinalValid row
  have he := row_historicalPaddedScheduleScalar row hrow hroot hbootCap hpLen
    hcover hcount hvalidPos hvalidLt
  change
    bootFuel * c.period ≤ c.rootSpan - 1 ∧ 1 + bootFuel * c.segLen < M ∧
    2 ≤ 1 + c.segLen - 1 ∧
    1 + bootFuel * c.segLen - 1 ≤ row.bootBound ∧
    ...
    (bootFuel + 1) * c.period + laterFuel * c.period + c.period = c.rootSpan ∧
    laterBase c bootFuel + laterFuel * c.segLen + c.segLen < M ∧
    laterBase c bootFuel + laterFuel * c.segLen + valid - 1 = c.rootCap ∧
  ...
  rcases he with ⟨heBootRange, heBootBase, heFirstLast, heBootEnd,
    heCrossRange, heCrossBase, heCrossStart, heCrossLast, heCrossCap,
    heCrossCover, heLaterRange, heLaterBase, heFinalIndex, heFinalBase,
    heFinalValid, heFinalCover, heMainBase⟩
  have hseg := row_historicalSegmentation row hrow hroot
  rcases hseg with ⟨hbootFloor, hbootNext, hfinalSeg⟩
  have hcount' : c.rootCount = bootFuel + 2 + laterFuel := by
    simpa only [c, rowCfg, bootFuel, laterFuel] using hcount
  ...
  have hprefixInv : PrimeTableInv
    ...
    rootLaterWindows_primeTable hcrossInv hlaterStart (by
      dsimp only [laterW, laterBase, crossW, crossingBase]
      omega)
  have hfinalEnd : laterW + laterFuel * c.segLen + valid - 1 =
      c.rootCap := by
    simpa only [laterW, valid, laterBase, crossingBase, Nat.add_assoc] using
      heFinalValid
  have hfinalStartNext : laterW + laterFuel * c.segLen =
      (laterW + laterFuel * c.segLen - 1) + 1 := by
    omega
  ...
  have hhistoricalInv : PrimeTableInv
      (rootScanFrom
        (rootLaterWindows c (crossingTable c row.bootBound bootFuel)
          (laterBase c bootFuel) laterFuel)
        (laterBase c bootFuel + laterFuel * c.segLen) valid) c.rootCap := by
    have hi := rootScanFrom_primeTable (fuel := valid) hprefixInv
      (w := laterW + laterFuel * c.segLen) (by omega) (by omega)
    simpa only [laterW, crossed, crossingTable, hfinalEnd] using hi
  have hfinalEq :
      rootScanFrom
        (rootLaterWindows c (crossingTable c row.bootBound bootFuel)
          (laterBase c bootFuel) laterFuel)
  ...
  refine
    { bootPrime := hboot
      ...
      laterCap := by
        ...
        have hadd := Nat.add_le_add_left hmul (laterBase c bootFuel)
        have hsub := Nat.sub_le_sub_right hadd 1
        have hstartCap : laterBase c bootFuel + laterFuel * c.segLen ≤
            c.rootCap := by omega
        exact Nat.le_trans hsub (Nat.le_trans (Nat.sub_le _ _) hstartCap)
      laterCover := by
        intro n hn'
        have hmul := Nat.mul_le_mul_right c.segLen
          (show n + 1 ≤ laterFuel by omega)
      ...
      finalIndex := heFinalIndex
      finalBaseM := heFinalBase
      finalValid := heFinalValid
      finalValidLt := by simpa only [valid] using hvalidLt
      finalCover := heFinalCover
      finalPrefixFit := by
        have hr := roomForStep_rootScanFrom_of_final hprefixInv hfinalStartNext
          hfinalStartTwo hfull.1 hfull.2 (k := 0) hfinalStartCap
      ...
      finalFit := by
        intro k hk
        have hkpred : k ≤ valid - 1 := Nat.le_pred_of_lt hk
        have hcap : laterW + laterFuel * c.segLen + k ≤ c.rootCap := by
          rw [← hfinalEnd]
          rw [Nat.add_sub_assoc (by omega : 1 ≤ valid)]
          simpa [Nat.add_assoc] using
            Nat.add_le_add_left hkpred (laterW + laterFuel * c.segLen)
        exact roomForStep_rootScanFrom_of_final hprefixInv hfinalStartNext
          hfinalStartTwo hfull.1 hfull.2 hcap
      finalCapFit := by
        rw [hfinalEq, hfull.2]
        simpa only [c] using (Nat.le_refl c.tableLen)
      finalLen := by rw [hfinalEq, hfull.2]
      finalPrime := hhistoricalInv
      firstPrimeLeCap := hn.firstPrimeLeCap
      finalBoundSqM := hn.rootCapSqM
      segFinalM := hn.segCapM
      mainIndexM := hn.mainIndexM
      mainBaseM := by
        rw [heMainBase]
        exact hn.mainBaseM }
...
window is padded after the literal `rootCap`. -/
theorem row_historicalPaddedCore_complete (row : Row) (hrow : row ∈ rows)
    (hroot : row.rootCount ≠ 1) (hbootCap : row.bootBound < row.rootCap)
    (hpLen : (rowCfg row).firstPrime ≤ (rowCfg row).segLen)
    (hcover : (rowCfg row).rootCap + 1 <
      (row.bootBound + 1) * (row.bootBound + 1))
    ...
    let ps := rootScanFrom
      (rootLaterWindows c
        (crossingTable c row.bootBound (historicalBootFuel row))
        (laterBase c (historicalBootFuel row)) (historicalLaterFuel row))
      (laterBase c (historicalBootFuel row) +
        historicalLaterFuel row * c.segLen)
      (historicalFinalValid row)
  ...
  let l := historicalLaterFuel row
  let valid := historicalFinalValid row
  have hs := row_historicalPaddedProductionSchedule row hrow hroot hbootCap
    hpLen hcover hcount hvalidPos hvalidLt
  have hsem := indexedPaddedProductionCore_complete c row.bootBound b l valid
  ...
  have hmainBase : mainBase c b l c.wDelta = c.lo := by
    have he := row_historicalPaddedScheduleScalar row hrow hroot hbootCap hpLen
      hcover hcount hvalidPos hvalidLt
    exact he.2.2.2.2.2.2.2.2.2.2.2.2.2.2.2.2
  change
    ...
    MachineTableRep c
      ...
      (rootScanFrom
        (rootLaterWindows c (crossingTable c row.bootBound b)
          (laterBase c b) l)
        (laterBase c b + l * c.segLen) valid) ∧
...
#print axioms row_historicalPaddedProductionSchedule
#print axioms row_historicalPaddedCore_complete
#print axioms row_historicalExactCore_complete
