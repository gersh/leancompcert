...
/-!
# Compiled Section 4.1.3 event body refines one reference slot
...
namespace LeanCompCert.Ports.Section413WindowBodyReferenceSound
...
theorem g1_first (k : Nat) (p : AState)
    ...
    (hr : EventReceipts k p g1Cfg rActive rS rQ rQ true false) :
    Accumulates p (afterFirst k p g1Cfg)
      (eventK1Contribution
        (divisorGate 1 (slotAt k).active (slotAt k).s)
        (referenceDiff g1G (slotAt k).q)
      ...
      (eventK2Contribution
        (divisorGate 1 (slotAt k).active (slotAt k).s)
        (referenceDiff g1G (slotAt k).q) (slotAt k).q false).lo
      (eventK2Contribution
        (divisorGate 1 (slotAt k).active (slotAt k).s)
        (referenceDiff g1G (slotAt k).q) (slotAt k).q false).hi := by
  ...
  have hqCap : (slotAt k).q ≤ g1TableCfg.cap := by
    exact Nat.le_trans hqN (Nat.le_trans hn (by decide))
  have hd := g1_tableDiff_eq_and_range hflag (slotAt k).q hqCap
  ...
  have h := event_clean_accumulates k p g1Cfg rActive rS rQ rQ
    ...
    (by simpa only [hq, harr, g1Cfg, g1TableCfg] using hd.2.1)
    (by simpa only [hq, harr, g1Cfg, g1TableCfg] using hd.2.2.1)
    (by simpa only [hq, harr, g1Cfg, g1TableCfg] using hd.2.2.2.1)
    (by simpa only [hq, harr, g1Cfg, g1TableCfg] using hd.2.2.2.2)
    hr
  have hd' : tableDiff g1Cfg.cap
      (LeanCompCert.Ports.Section413G1TableProgram.rawFinal g1TableCfg).arr
      (slotAt k).q = referenceDiff g1G (slotAt k).q := by
    simpa only [g1Cfg, g1TableCfg, referenceDiff] using hd.1
  rw [hs, hq, ha, harr] at h
  rw [hd'] at h
  simpa only [afterFirst, firstEvent, g1Cfg, referenceDiff] using h

set_option maxRecDepth 100000 in
theorem g1_second (k : Nat) (p : AState)
    ...
    (hflag : g1TableCfg.tFlag = 0)
    (hadd : p.regs LeanCompCert.Ports.Section413SignedAdd.rViol = 0)
    (hr : EventReceipts k p g1Cfg rPair rQ rS rS true false) :
    Accumulates p (arun k p (secondEvent g1Cfg))
      (eventK1Contribution
        (divisorGate 1 (slotAt k).pair (slotAt k).q)
        (referenceDiff g1G (slotAt k).s)
      ...
      (eventK2Contribution
        (divisorGate 1 (slotAt k).pair (slotAt k).q)
        (referenceDiff g1G (slotAt k).s) (slotAt k).s false).lo
      (eventK2Contribution
        (divisorGate 1 (slotAt k).pair (slotAt k).q)
        (referenceDiff g1G (slotAt k).s) (slotAt k).s false).hi := by
  have hsCap : (slotAt k).s ≤ g1TableCfg.cap := by
  ...
  have hd := g1_tableDiff_eq_and_range hflag (slotAt k).s hsCap
  have hactiveBit : p.regs rPair = 0 ∨ p.regs rPair = 1 := by
    ...
    by_cases h : (k % slots + 1) * (k % slots + 1) ≤ k / slots + 1 ∧
      ...
      simp [slotAt, h]
  have h := event_clean_accumulates k p g1Cfg rPair rQ rS rS true false
    hword harray (by decide) (by decide) hactiveBit
    (by decide) (by decide) (by decide)
    (by rw [hs]; exact hsCap)
    (by decide) (by decide) (by decide) (by decide) hadd
    (by decide) (by decide) (by decide) (by decide)
    (by simpa only [hs, harr, g1Cfg, g1TableCfg] using hd.2.1)
    (by simpa only [hs, harr, g1Cfg, g1TableCfg] using hd.2.2.1)
    (by simpa only [hs, harr, g1Cfg, g1TableCfg] using hd.2.2.2.1)
    (by simpa only [hs, harr, g1Cfg, g1TableCfg] using hd.2.2.2.2)
    hr
  have hd' : tableDiff g1Cfg.cap
      (LeanCompCert.Ports.Section413G1TableProgram.rawFinal g1TableCfg).arr
      (slotAt k).s = referenceDiff g1G (slotAt k).s := by
    simpa only [g1Cfg, g1TableCfg, referenceDiff] using hd.1
  rw [hs, hq, ha, harr] at h
  rw [hd'] at h
  simpa only [secondEvent, g1Cfg, referenceDiff] using h

set_option maxRecDepth 100000 in
theorem g1_third (k : Nat) (p : AState)
    (hk : k < productionRows * slots)
    (hword : ∀ j, p.regs j < M) (harray : ∀ j, p.arr j < M)
    (hs : p.regs rS = (slotAt k).s)
    ...
    (hflag : g1TableCfg.tFlag = 0)
    (hadd : p.regs LeanCompCert.Ports.Section413SignedAdd.rViol = 0)
    (hr : EventTwiceReceipts k p g1Cfg rHalfActive rS rHalfQ rHalfQ
      false true) :
    Accumulates p (arun k p (thirdEvent g1Cfg))
      (eventK1Contribution
        (divisorGate 1 (slotAt k).halfActive (slotAt k).s)
        (referenceDiff g1G (slotAt k).halfQ)
      ...
      (eventK2Contribution
        (divisorGate 1 (slotAt k).halfActive (slotAt k).s)
        (referenceDiff g1G (slotAt k).halfQ) (slotAt k).halfQ true).lo
      (eventK2Contribution
        (divisorGate 1 (slotAt k).halfActive (slotAt k).s)
        (referenceDiff g1G (slotAt k).halfQ) (slotAt k).halfQ true).hi := by
  have hn : (slotAt k).n ≤ productionRows := by
    simp only [slotAt, productionRows, slots] at hk ⊢
    omega
  ...
  have hqCap : (slotAt k).halfQ ≤ g1TableCfg.cap :=
    Nat.le_trans hqN (Nat.le_trans hn (by decide))
  have hd := g1_tableDiff_eq_and_range hflag (slotAt k).halfQ hqCap
  ...
  have h := eventTwice_clean_accumulates k p g1Cfg rHalfActive rS rHalfQ
  ...
  have hd' : tableDiff g1Cfg.cap
      (LeanCompCert.Ports.Section413G1TableProgram.rawFinal g1TableCfg).arr
      (slotAt k).halfQ = referenceDiff g1G (slotAt k).halfQ := by
    simpa only [g1Cfg, g1TableCfg, referenceDiff] using hd.1
  rw [hs, hq, ha, harr] at h
  rw [hd'] at h
  simpa only [thirdEvent, g1Cfg, referenceDiff] using h

set_option maxRecDepth 100000 in
theorem g1_fourth (k : Nat) (p : AState)
    (hk : k < productionRows * slots)
    (hword : ∀ j, p.regs j < M) (harray : ∀ j, p.arr j < M)
    (hs : p.regs rS = (slotAt k).s)
    (hq : p.regs rHalfQ = (slotAt k).halfQ)
    ...
    (hflag : g1TableCfg.tFlag = 0)
    (hadd : p.regs LeanCompCert.Ports.Section413SignedAdd.rViol = 0)
    (hr : EventTwiceReceipts k p g1Cfg rHalfPair rHalfQ rS rS false true) :
    Accumulates p (arun k p (fourthEvent g1Cfg))
      (eventK1Contribution
        (divisorGate 1 (slotAt k).halfPair (slotAt k).halfQ)
        (referenceDiff g1G (slotAt k).s)
      ...
      (eventK2Contribution
        (divisorGate 1 (slotAt k).halfPair (slotAt k).halfQ)
        (referenceDiff g1G (slotAt k).s) (slotAt k).s true).lo
      (eventK2Contribution
        (divisorGate 1 (slotAt k).halfPair (slotAt k).halfQ)
        (referenceDiff g1G (slotAt k).s) (slotAt k).s true).hi := by
  have hn : (slotAt k).n ≤ productionRows := by
    simp only [slotAt, productionRows, slots] at hk ⊢
    omega
  have hsCap : (slotAt k).s ≤ g1TableCfg.cap := by
  ...
  have htwice : safeDen (p.regs rHalfQ) * 2 < M := by
    ...
    have hsafe : safeDen (slotAt k).halfQ ≤ productionRows := by
      ...
      · exact hqBound
    exact Nat.lt_of_le_of_lt (Nat.mul_le_mul_right 2 hsafe) (by decide)
  have h := eventTwice_clean_accumulates k p g1Cfg rHalfPair rHalfQ rS rS
  ...
  rw [hs, hq, ha, harr] at h
  rw [hd'] at h
  simpa only [fourthEvent, g1Cfg, referenceDiff] using h

set_option maxRecDepth 100000 in
theorem g1_body_slot (k : Nat) (p : AState)
    (hk : k < productionRows * slots)
    (hword : ∀ j, p.regs j < M) (harray : ∀ j, p.arr j < M)
    (hsched :
    ...
    Accumulates p (arun k p (eventBody g1Cfg))
  ...
  let p1 := afterFirst k p g1Cfg
  let p2 := afterSecond k p g1Cfg
  let p3 := afterThird k p g1Cfg
  ...
  have frame1 (r : Nat) (hw : LeanCompCert.Verified.ArrayRegFrame.writes r
      (firstEvent g1Cfg) = false) : p1.regs r = p.regs r := by
  ...
  have frame2 (r : Nat) (hw : LeanCompCert.Verified.ArrayRegFrame.writes r
      (firstEvent g1Cfg ++ secondEvent g1Cfg) = false) :
  ...
  have frame3 (r : Nat) (hw : LeanCompCert.Verified.ArrayRegFrame.writes r
      (firstEvent g1Cfg ++ secondEvent g1Cfg ++ thirdEvent g1Cfg) = false) :
      p3.regs r = p.regs r := by
    simpa only [p3, p2, p1, afterThird, afterSecond, afterFirst,
      arun_append] using
      LeanCompCert.Verified.ArrayRegFrame.arun_frame k r _ hw p
  have h1 := g1_first k p hk hword harray hsched.2.1 hsched.2.2.1
    hsched.2.2.2.1 harr hflag hadd hr.first
  have h2 := g1_second k p1 hk hp1word hp1array
  ...
  have h3 := g1_third k p2 hk hp2word hp2array
  ...
  have h4 := g1_fourth k p3 hk hp3word hp3array
  ...
  have hbody := eventBody_accumulates k p g1Cfg _ _ _ _ _ _ _ _ _ _ _ _
    _ _ _ _ h1 h2 h3 h4
  have hok1 := ordinaryK1 g1G 1 (slotAt k).n (k % slots) (Or.inl rfl)
  have hok2 := ordinaryK2 g1G 1 (slotAt k).n (k % slots) (Or.inl rfl)
  have hhk1 := halfK1 g1G 1 (slotAt k).n (k % slots) (Or.inl rfl)
  have hhk2 := halfK2 g1G 1 (slotAt k).n (k % slots) (Or.inl rfl)
  let a1 := eventK1Contribution
    (divisorGate 1 (slotAt k).active (slotAt k).s)
    (referenceDiff g1G (slotAt k).q) (safeDen (slotAt k).s) true
  let a2 := eventK1Contribution
    (divisorGate 1 (slotAt k).pair (slotAt k).q)
    (referenceDiff g1G (slotAt k).s) (safeDen (slotAt k).q) true
  let a3 := eventK1Contribution
    (divisorGate 1 (slotAt k).halfActive (slotAt k).s)
    (referenceDiff g1G (slotAt k).halfQ) (safeDen (slotAt k).s * 2) false
  let a4 := eventK1Contribution
    (divisorGate 1 (slotAt k).halfPair (slotAt k).halfQ)
    (referenceDiff g1G (slotAt k).s) (safeDen (slotAt k).halfQ * 2) false
  let b1 := eventK2Contribution
    (divisorGate 1 (slotAt k).active (slotAt k).s)
    (referenceDiff g1G (slotAt k).q) (slotAt k).q false
  let b2 := eventK2Contribution
    (divisorGate 1 (slotAt k).pair (slotAt k).q)
    (referenceDiff g1G (slotAt k).s) (slotAt k).s false
  let b3 := eventK2Contribution
    (divisorGate 1 (slotAt k).halfActive (slotAt k).s)
    (referenceDiff g1G (slotAt k).halfQ) (slotAt k).halfQ true
  let b4 := eventK2Contribution
    (divisorGate 1 (slotAt k).halfPair (slotAt k).halfQ)
    (referenceDiff g1G (slotAt k).s) (slotAt k).s true
  ...
  have hk1lo : (((a1.lo + a2.lo) + a3.lo) + a4.lo) =
      (LeanCompCert.Ports.Section413WindowPairingBridge.k1SlotDelta
        g1G 1 (slotAt k).n (k % slots)).lo := by
    calc
      (((a1.lo + a2.lo) + a3.lo) + a4.lo) =
          (cadd (cadd a1 a2) (cadd a3 a4)).lo := by simp [cadd, Int.add_assoc]
      _ = (cadd
          (LeanCompCert.Ports.Section413WindowPairingBridge.slotDelta 1
            (slotAt k).n (k1First g1G (slotAt k).n) (k % slots))
          (if (slotAt k).n % 2 = 0 then
            LeanCompCert.Ports.Section413WindowPairingBridge.slotDelta 1
              ((slotAt k).n / 2) (k1Second g1G ((slotAt k).n / 2))
  ...
  have hk2lo : (((b1.lo + b2.lo) + b3.lo) + b4.lo) =
    ...
    calc
      ...
      _ = (cadd
          (LeanCompCert.Ports.Section413WindowPairingBridge.slotDelta 1
            (slotAt k).n (k2First g1G (slotAt k).n) (k % slots))
          (if (slotAt k).n % 2 = 0 then
            LeanCompCert.Ports.Section413WindowPairingBridge.slotDelta 1
              ((slotAt k).n / 2) (k2Second g1G ((slotAt k).n / 2))
...
#print axioms g1_first
#print axioms g1_second
#print axioms g1_third
#print axioms g1_fourth
#print axioms g1_body_slot

end LeanCompCert.Ports.Section413WindowBodyReferenceSound
