...
The cheap independent emit-time checks are isolated in `R2SegSieveChecks`.
The old closed whole-program kernel reductions were not proof dependencies
and were retired after the analogous `ψ` check demonstrated the
multi-gigabyte rebuild cost.  The production route uses the proved
compiler/denotation chain above and compiled agreement against `bench/ref_r2.c`
at `S = 24` and `hi = 10⁶, 10⁷, 10⁸`.
-/
