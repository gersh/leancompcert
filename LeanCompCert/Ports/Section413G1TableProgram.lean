import LeanCompCert.Ports.Section413G1FinalDenote
import LeanCompCert.Ports.Section413G1Sound

/-!
# A verified-code carrier for a complete Section 4.13 `g₂` table

The already verified `Section413G1Program` computes the fixed-point `g₂`
...
machine/transparent-model proof in `Section413G1FinalDenote` remains the
...
namespace LeanCompCert.Ports.Section413G1TableProgram
...
theorem base_init_wf (c : Cfg) : ∀ i ∈ c.init, i.WF regCount := by
  ...
  exact ainstr_wf_mono (by decide :
    LeanCompCert.Ports.Section413G1Program.regCount ≤ regCount)
    i ((g1Program_wf c).2.1 i (by simpa [g1Program] using hi))

theorem base_body_wf (c : Cfg) : ∀ i ∈ c.body, i.WF regCount := by
  intro i hi
  exact ainstr_wf_mono (by decide :
    LeanCompCert.Ports.Section413G1Program.regCount ≤ regCount)
    i ((g1Program_wf c).2.2.1 i (by simpa [g1Program] using hi))
...
abbrev DAdmissible := LeanCompCert.Ports.Section413G1Denote.Admissible
...
theorem tableStoreBody_defined (c : Cfg) (k : Nat) (s : AState) :
  ...
  have hprep : (tableStorePrep c).all
      LeanCompCert.Ports.Section413G1Denote.alwaysDef = true := by
    simp [tableStorePrep, LeanCompCert.Ports.Section413G1Denote.alwaysDef,
      hmod, Nat.ne_of_gt hp]
  have hd := LeanCompCert.Ports.Section413G1Denote.allDefined_of_alwaysDef
...
theorem extended_body_defined (c : Cfg) (hc : TableAdmissible c) (idx : Nat)
  ...
  have hb0 := LeanCompCert.Ports.Section413G1Denote.body_defined
...
theorem checkWord_arr (address expected : Nat) (s : AState) :
  ...
  apply LeanCompCert.Ports.Section413G1Denote.arun_arr_frame
  intro i hi
  simp only [checkWord, List.mem_cons, List.not_mem_nil, or_false] at hi
  rcases hi with rfl | rfl | rfl | rfl <;>
    rfl
...
theorem extended_body_wordInv (c : Cfg) (idx : Nat) (s : AState)
  ...
  exact LeanCompCert.Ports.Section413G1Denote.arun_lt idx
