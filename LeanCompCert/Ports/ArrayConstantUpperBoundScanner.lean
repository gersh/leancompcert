...
/-!
# External constant-bound array shard scanner
...
when `arr[i] > bound`.  It never executes the producer computation.
-/

set_option autoImplicit false

namespace LeanCompCert.Ports.ArrayConstantUpperBoundScanner
...
def body (c : Cfg) : List AInstr :=
  ...
  , .load rProd rAddr
  , .scalar (.mov rCandidate (.lit c.bound))
...
structure Ready (c : Cfg) (arr : Nat → Nat) : Prop where
  ...
  candidate_word : c.bound < M
  words : ∀ i, arr i < M

def ShardChecked (c : Cfg) (arr : Nat → Nat) : Prop :=
  ∀ i, c.iLo ≤ i → i < c.iHi → arr i ≤ c.bound
...
theorem body_bad (c : Cfg) (arr : Nat → Nat) (k : Nat) (s : AState)
    ...
    (arun k s (body c)).regs rBad =
      (s.regs rBad |||
        (if arr (c.iLo + k) > c.bound then 1 else 0)) % M := by
...
end LeanCompCert.Ports.ArrayConstantUpperBoundScanner
