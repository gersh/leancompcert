...
/-!
# Source observation for the Proposition 12.2.4 cell program

The cell campaign stores eleven result words.  This module identifies them with
the exact source registers after the finite sweep; it performs no production
computation in Lean.
-/
...
def resultRegs : List Nat :=
  [rG, rCells, rMin, rT, rEx, rViol] ++ violRegs

theorem epilogue_eq_resultStores (c : CellCfg) :
    c.epilogue = storeResults c 0 resultRegs := by
  rfl

theorem arun_storeResult_arr_self (c : CellCfg) (slot reg k : Nat)
...
theorem arun_storeResult_reg_frame (c : CellCfg) (slot reg keep k : Nat)
...
theorem arun_storeResult_arr_ne (c : CellCfg) (slot reg cell k : Nat)
...
theorem arun_storeResults_reg_frame (c : CellCfg) (slot k keep : Nat)
...
theorem arun_storeResults_arr_before (c : CellCfg) (slot k cell : Nat)
...
theorem arun_storeResults_arr_get (c : CellCfg) (slot k i : Nat)
...
theorem epilogue_result_cell (c : CellCfg) (s : AState) (i : Nat)
