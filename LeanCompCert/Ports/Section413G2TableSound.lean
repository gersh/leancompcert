...
namespace LeanCompCert.Ports.Section413G2TableSound
...
def CoreEq (c : Cfg) (a b : AState) : Prop :=
  (∀ j, j < LeanCompCert.Ports.Section413G2Program.regCount →
    a.regs j = b.regs j) ∧
...
theorem adefined_core {c : Cfg} {k : Nat} {a b : AState} {i : AInstr}
    (heq : CoreEq c a b)
    (hwf : i.WF LeanCompCert.Ports.Section413G2Program.regCount)
...
theorem arun_core {c : Cfg} {k : Nat} :
    ∀ (l : List AInstr) (a b : AState),
      CoreEq c a b →
      (∀ i ∈ l, i.WF LeanCompCert.Ports.Section413G2Program.regCount) →
...
theorem body_wf256 (c : Cfg) :
    ∀ i ∈ c.body, i.WF LeanCompCert.Ports.Section413G2Program.regCount := by
  intro i hi
  exact (g2Program_wf c).2.2.1 i (by simpa [g2Program] using hi)

theorem finBody_regs_31_35 (c : Cfg) (k : Nat) (s : AState) (j : Nat)
    (hj : j = 31 ∨ j = 35) :
    (arun k s c.finBody).regs j = s.regs j := by
  apply LeanCompCert.Ports.Section413G2Denote.arun_regs_frame
  intro i hi
  have hall : c.finBody.all (fun x => decide
      (LeanCompCert.Ports.Section413G2Denote.adest x ≠ some j)) = true := by
    rcases hj with rfl | rfl <;> rfl
  exact of_decide_eq_true ((List.all_eq_true.mp hall) i hi)

set_option maxRecDepth 10000 in
theorem body_selector (c : Cfg)
    (hc : LeanCompCert.Ports.Section413G2Denote.Admissible c)
    (idx : Nat) (hidx : idx < c.loopCount) (s : AState) :
    let p := arun idx s c.body
    p.regs 31 = (c.tsel idx).X ∧ p.regs 35 = (c.tsel idx).isF := by
  rw [LeanCompCert.Ports.Section413G2Denote.body_eq_prefix_fin, arun_append]
  have h := LeanCompCert.Ports.Section413G2Denote.selTrialPassAWeightPassB_selector
    c hc idx hidx s
...
theorem tsel_store_product_le (c : Cfg)
  ...
  · have hphase : c.phase1 ≤ idx := Nat.le_of_not_gt hp
    ...
    have hpPos : 0 < c.p := LeanCompCert.Ports.Section413G2Denote.p_pos c
    ...
    have hX : (c.tsel idx).X ≤ c.cap := by
      ...
      omega
    have hF := LeanCompCert.Ports.Section413G2Denote.tsel_isF_le_one c idx
...
theorem tableStore_core (c : Cfg) (hc : TableAdmissible c)
  ...
  · intro j hj
    ...
    rcases hi with
      ...
      simp [LeanCompCert.Ports.Section413G2Denote.adest, rAddrLo, rAddrHi,
        rValLo, rValHi] <;> omega
  · intro i hi
...
def PairInv (c : Cfg) (a b : AState) : Prop :=
  CoreEq c a b ∧
  LeanCompCert.Ports.Section413G2Denote.MachineInv b ∧ WordInv a

theorem entry_pairInv (c : Cfg) (hc : TableAdmissible c) :
    PairInv c (rawEntry c) (rawEntry c) := by
  refine ⟨⟨fun _ _ => rfl, fun _ _ => rfl⟩, ?_, ?_⟩
  · exact LeanCompCert.Ports.Section413G2Denote.machineEntry_inv c hc.base
...
theorem pair_step (c : Cfg) (hc : TableAdmissible c) (idx : Nat)
  ...
  have hbdef := LeanCompCert.Ports.Section413G2Denote.body_defined
  ...
  · exact LeanCompCert.Ports.Section413G2Denote.body_machineInv
...
theorem basePrefix_obs (c : Cfg) (hc : TableAdmissible c) :
    ∀ k, k ≤ c.loopCount →
      LeanCompCert.Ports.Section413G2Denote.obsT (basePrefix c k) =
        LeanCompCert.Ports.Section413G2Sound.tRunUpto c k := by
  intro k
  induction k with
  | zero =>
      intro hk
      ...
      simpa only [basePrefix, rawEntry, List.range_zero, List.foldl_nil,
        LeanCompCert.Ports.Section413G2Sound.tRunUpto_zero] using hi
  | succ k ih =>
      intro hk
      have hp := parallel_prefix c hc k (by omega)
      have hInv := hp.2.1
      have hbody := LeanCompCert.Ports.Section413G2Denote.body_obs
        c hc.base k (by omega) (basePrefix c k) hInv.1 hInv.2.1
        hInv.2.2.1 hInv.2.2.2.1 hInv.2.2.2.2
      rw [LeanCompCert.Ports.Section413G2Sound.tRunUpto_succ, ← ih (by omega)]
      rw [basePrefix_succ]
...
theorem rawFinal_core_tRun (c : Cfg) (hc : TableAdmissible c) :
  ...
  rw [← LeanCompCert.Ports.Section413G2Sound.tRun_eq_tRunUpto] at hobs
...
theorem rawStep_arr_high (c : Cfg) (hc : TableAdmissible c) (idx : Nat)
  ...
  let p := arun idx s c.body
  have hdef := LeanCompCert.Ports.Section413G2Denote.body_defined
...
theorem tsel_X_pos_of_isF_one (c : Cfg) (hc : TableAdmissible c)
  ...
  have hsub : tsub idx c.phase1 = idx - c.phase1 := by
    ...
    rw [heq, Nat.add_mod_right, Nat.mod_eq_of_lt hsubM]
  have hpPos : 0 < c.p :=
    LeanCompCert.Ports.Section413G2Denote.p_pos c
  have htail : idx - c.phase1 < c.cap * c.p := by
...
theorem rawPrefix_zero_cells (c : Cfg) (hc : TableAdmissible c) :
  ...
  | zero =>
      ...
      simp [rawPrefix, rawEntry, Cfg.init,
        ...
        hlo1, hhi1, show 1 % M = 1 by decide]
  | succ k ih =>
      intro hk
      ...
      · have hF0 : (c.tsel k).isF = 0 := by
          have hFle :=
            LeanCompCert.Ports.Section413G2Denote.tsel_isF_le_one c k
...
theorem rawPrefix_boundary_cell (c : Cfg) (hc : TableAdmissible c)
    (hsound : LeanCompCert.Ports.Section413G2Sound.Admissible c)
    (X : Nat) (hX : 1 ≤ X) (hXc : X ≤ c.cap) :
    (rawPrefix c (c.phase1 + X * c.p)).arr (tableLo c + X) =
        (LeanCompCert.Ports.Section413G2Sound.tRunUpto c
          (c.phase1 + X * c.p)).gLo ∧
  ...
  have hz0 := LeanCompCert.Ports.Section413G2Sound.tsel_final_block
    c hsound (X - 1) hXm1
...
theorem flagStep_eq_zero_iff {v : Nat} (hv : v < M) (bad : Prop)
  ...
  rw [flagStep, LeanCompCert.Ports.Section413G2Denote.lor_mod_M hv
    (LeanCompCert.Ports.Section413G2Denote.bnat_lt_M bad)]
  constructor
  · intro h
    have hz := LeanCompCert.Ports.Section413G2Sound.or_eq_zero h
    exact ⟨hz.1, LeanCompCert.Ports.Section413G2Sound.bnat_eq_zero_iff.mp hz.2⟩
  · rintro ⟨rfl, hbad⟩
    simp [LeanCompCert.Ports.Section413G2Sound.bnat_false hbad]
...
theorem cellsOfWords_canonical (c : Cfg) (words : Array (UInt64 × UInt64)) :
  ...
  · have hcell : (cellsOfWords words)[X]! =
    ...
    rw [hcell]
    constructor
    · rw [LeanCompCert.Ports.Section413G2Sound.encodeZ_decodeZ_self]
...
theorem rawValue_zero_expected_g2 (c : Cfg) (expected : Array Cell)
    (hc : TableAdmissible c)
    (hsound : LeanCompCert.Ports.Section413G2Sound.Admissible c)
  ...
  have hloEnc := LeanCompCert.Ports.Section413G2Sound.encodeZ_lt_M expected[X]!.lo
  have hhiEnc := LeanCompCert.Ports.Section413G2Sound.encodeZ_lt_M expected[X]!.hi
...
theorem compiled_zero_expected_g2 (c : Cfg) (expected : Array Cell)
    (hc : TableAdmissible c)
    (hsound : LeanCompCert.Ports.Section413G2Sound.Admissible c)
    (hcanonical : ExpectedCanonical c expected)
  ...
  exact rawValue_zero_expected_g2 c expected hc hsound hcanonical hz


/-- Sharded counterpart of `compiled_zero_expected_g2`: `n` artifacts, each
verifying `q` consecutive cells with an identical sweep, together entail exactly
what the single full-table artifact entails. -/
theorem compiled_zero_expected_g2_even_shards (c : Cfg) (expected : Array Cell)
    (hc : TableAdmissible c)
    (hsound : LeanCompCert.Ports.Section413G2Sound.Admissible c)
    (hcanonical : ExpectedCanonical c expected)
    ...
    c.tFlag = 0 ∧ ∀ X, 1 ≤ X → X ≤ c.cap →
      expected[X]! =
        (LeanCompCert.Ports.Section413G2Sound.g2Prefix
          c.rounds c.checkLo c.cap X).g := by
  refine rawValue_zero_expected_g2 c expected hc hsound hcanonical ?_
...
end LeanCompCert.Ports.Section413G2TableSound
