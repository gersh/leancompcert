import LeanCompCert.Ports.Section413G1Head10000Certificate
...
open LeanCompCert.Ports.Section413G1Head10000Certificate
...
def main (args : List String) : IO UInt32 := do
  ...
  let some expected :=
      ...
      else if mode = "control" then some (expected10000.set! (start + 1) czero)
      else none
    | do
      IO.eprintln "mode must be positive or control"
      return 1
  let p := tableProgramOn headCfg expected idxs
  let suffix := if n = 1 then "" else s!"_s{k}of{n}"
  let name := (if mode = "positive" then "S413G1Table10000" else
    "S413G1Table10000Control") ++ suffix
